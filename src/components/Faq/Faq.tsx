...
const Faq = () => {
  ...
  return (
    <section className="firstSection rounded-xl w-full" ref={scope}>
      <motion.div className="relative w-full h-full pb-10" ref={ref}>
        <motion.img
          src={Header}
        ...
        <div className="absolute inset-0 flex z-20 flex-col p-4 justify-center pb-20 items-center gap-10">
          <p className="font-topicItems font-extrabold text-yellow-300  text-4xl md:text-6xl rounded opacity-0">
            F.A.Q
          </p>
          <div className="flex grow flex-col gap-2 max-w-2xl">
            {faq.map((item: FAQItem, index: number) => (
              <div key={index} className="question opacity-0 faq-item w-full relative">
                <FontAwesomeIcon
                ...
                />
                <button
                  className={`text-left text-white faq-question shadow-lg text-sm sm:text-lg  w-full font-display bg-zinc-900 p-2 rounded-md font-semibold transition-all duration-500 pr-6 ${
                    activeQuestion === index ? "rounded-b-none" : ""
                  }`}
                  onClick={() => toggleQuestion(index)}
                >
                  {item.question}
                ...
                <div
                  ...
                  className={`font-display text-light faq-answer text-sm sm:text-base bg-zinc-700 ${
                    activeQuestion === index
                      ? "show p-4 rounded-b-md text-white opacity-1"
                      : "opacity-0"
                  }`}
                ></div>
              </div>
